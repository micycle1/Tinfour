...
public interface IQuadEdge {
 ...
 /**
   ...
   * @param l2d a valid GeoLine instance to receive the geometry data from the edge.
   */
  @Deprecated
  void setLine2D(GeoAffineTransform transform, GeoLine l2d);


  /**
   * Provides a convenience method for rendering edges by setting the
   * GeoLine argument with the transformed coordinates of the edge.
   ...
   * are then stored in the application-supplied GeoLine object.  If a null
   ...
   * render a large number of edges using Java's GeoLine class. In such cases,
   * this method avoids the overhead involved in creating multiple GeoLine
   * instances by allowing an application to reuse a single instance multiple
   * times.
   * @param transform a valid affine transform
   * @param l2d a valid GeoLine instance to receive the geometry data from the edge.
   */
  void transcribeToLine2D(GeoAffineTransform transform, GeoLine l2d);
